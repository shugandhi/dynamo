...
Disaggregated prefill/decode | Not yet supported.
High availability routing features | Restart-stable kv cache, Protection against workers events loss not supported 
Cross-replica KV-index warm-up | Not wired; a new replica re-warms from live traffic + replay.
Per-tenant KV cache isolation x-tenant-id / cache_salt | KV routing per tenant is not supported at this time.This requires per-engine support
Request migration, rejection, cancellation - overall admission control | Not supported
Topology / Zone aware routing  | Not supported
Speculative Decoding awareness | Not supported
Session affinity | Not supported
Full list of OpenAI HTTP endpoints|  Not supported
Multi-modal | Not supported 
Metrics/observability | Not yet supported
