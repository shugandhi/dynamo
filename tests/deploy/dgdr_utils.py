...
def unique_name(config_: DGDRTestConfig, suffix: str) -> str:
    ...
    digest = hashlib.sha1(suffix.encode(), usedforsecurity=False).hexdigest()[:6]
    if config_.name_prefix:
        return f"{config_.name_prefix[:37]}-{digest}"
    millis = time.time_ns() // 1_000_000 % 100000
    return f"dgdr-test-{suffix[:25]}-{millis}-{digest}"
