...
/// Regression: `stream: true` with a synchronous `Backend(InvalidArgument)` at
/// t=0 must return HTTP 400 *before* HTTP 200 is committed, not HTTP 200
/// followed by a generic 500 SSE frame.
#[tokio::test]
async fn test_streaming_chat_completions_returns_4xx_on_backend_invalid_argument() {
    let (listener, port) = bind_random_port().await;
    let service = HttpService::builder()
        .port(port)
        .enable_chat_endpoints(true)
    ...
    assert_eq!(
        ...
        "streaming Backend(InvalidArgument) must land as HTTP 400 before HTTP 200 is committed; got {status}, body: {body}"
    );
    assert!(
        body.contains("Received multimodal data but multimodal processing is not enabled"),
        "expected typed backend error message forwarded to client; got: {body}"
    );
