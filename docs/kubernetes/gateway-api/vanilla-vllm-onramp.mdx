...
sidebar-title: "Vanilla vLLM On-ramp without Dynamo runtime"
subtitle: Dynamo EPP routing for existing GAIE deployments that run stock vLLM pods.
---
This on-ramp is for Kubernetes stacks that already use
[Gateway API Inference Extension (GAIE)](https://github.com/kubernetes-sigs/gateway-api-inference-extension) with
stock `vLLM serve` pods, or that want to evaluate the Dynamo kv aware router without installing the Dynamo operator
...
with the Dynamo EPP image, specify the DYN_EPP_MODE="standalone" and you are done — no need to install the Dynamo operator, Dynamo runtime, or Dynamo worker.
...
[`vllm-qwen-pool` InferencePool in `agg.yaml`](../../../deploy/inference-gateway/ext-proc/examples/onramp/agg.yaml#L254) for a complete example):
...
[`vllm-qwen-route` HTTPRoute in `agg.yaml`](../../../deploy/inference-gateway/ext-proc/examples/onramp/agg.yaml#L270) for a complete example):
...
[`dynamo-epp` Deployment in `agg.yaml`](../../../deploy/inference-gateway/ext-proc/examples/onramp/agg.yaml#L146) for a complete example) and set:
